...
const NeuralNetworkAnimation: React.FC<NeuralNetworkAnimationProps> = ({
    ...
    activationColor = '#818cf8', // Indigo-400 (Brighter for dark mode)
    ...
    useEffect(() => {
        const animate = () => {
            ...
            setTimeout(() => {
                ...
                if (outputEl) (outputEl as SVGElement).style.opacity = '0.7';
            }, currentTime);
            ...
            setTimeout(() => {
                ...
                svgRef.current?.querySelectorAll('.connection').forEach(conn => {
                    ...
                    (conn as SVGElement).style.opacity = '0.1';
                });
    ...
    const layers = {
        input: { neurons: 4, x: 100 },
        hidden: { neurons: 3, x: 280 },
        output: { neurons: 1, x: 440 }
    };
    ...
    const getYPosition = (layerSize: number, index: number): number => {
        const spacing = (svgHeight - 100) / (layerSize + 1);
    ...
    return (
        <div style={{ width: '100%', maxWidth: '800px', backgroundColor }}>
            <svg
                ref={svgRef}
                width="100%"
